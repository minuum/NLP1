...
# Import the custom DCG model from training script
import sys
sys.path.append(os.path.dirname(__file__))
from rp_dcg_train_3 import ImprovedT5WithDCG

# Enhanced Configuration for DCG Model 3
CONFIG = {
    "model_dir": "./improved_dcg_model",
    "data_dir": "./data", 
    "results_dir": "./results",
    "batch_size": 8,
    "generation_params": {
...
class DCGTitleGenerationEvaluator:
    """DCG 제목 생성 평가 시스템"""
    
    def __init__(self, model_name="Improved DCG T5"):
    ...
    def load_model(self, model_path=None):
        ...
        print(f"🔄 Loading DCG model from {model_path}...")
        ...
        try:
            ...
            # SafeTensors 우선 시도, 그 다음 PyTorch 모델 시도
            try:
                ...
                if os.path.exists(safetensors_path):
                    ...
                    print("✅ DCG Model loaded from SafeTensors")
                else:
                    raise FileNotFoundError("SafeTensors not found")
            except Exception as st_e:
                print(f"⚠️  SafeTensors loading failed: {st_e}")
                # PyTorch 모델 시도
                pytorch_path = os.path.join(model_path, "pytorch_model.bin")
                if os.path.exists(pytorch_path):
                    model.load_state_dict(torch.load(pytorch_path, map_location="cpu"), strict=False)
                    print("✅ DCG Model loaded from PyTorch")
                else:
            ...
            if checkpoint_dirs:
                ...
                # SafeTensors 또는 PyTorch 모델 로드 시도
                try:
                    from safetensors.torch import load_file
                    ...
                    if os.path.exists(safetensors_path):
                        state_dict = load_file(safetensors_path)
                        model.load_state_dict(state_dict, strict=False)
                        print("✅ DCG Checkpoint loaded from SafeTensors")
                    else:
                        raise FileNotFoundError("Checkpoint SafeTensors not found")
                except Exception as st_e:
                    print(f"⚠️  Checkpoint SafeTensors loading failed: {st_e}")
                    pytorch_path = os.path.join(latest_checkpoint, "pytorch_model.bin")
                    if os.path.exists(pytorch_path):
                        model.load_state_dict(torch.load(pytorch_path, map_location="cpu"), strict=False)
                        print("✅ DCG Checkpoint loaded from PyTorch")
                    else:
                        raise FileNotFoundError("Checkpoint PyTorch model not found")
            else:
    ...
    def generate_titles(self, model, tokenizer, test_data):
        """제목 생성"""
        print("🎯 Generating titles with DCG model...")
    ...
    def create_evaluation_plots(self, metrics, generated, references):
        ...
        try:
            ...
            print(f"⚠️  Cannot display plot interactively: {e}")
            print(f"📊 Please check the saved plot at: {plot_path}")
    ...
    def print_results(self, metrics, generated, references):
        ...
        # DCG 특성
        print(f"\n🔧 DCG Model Features:")
        print(f"  • Dynamic Context Gates enabled")
        print(f"  • Simplified gate architecture")
        print(f"  • Gate regularization applied")
        print(f"  • Progressive DCG activation")
...
def evaluate_dcg_model():
    """DCG T5 모델 평가"""
    evaluator = DCGTitleGenerationEvaluator("Improved DCG T5")

"""
SimpleDCG: 단순화된 Dynamic Context Gates를 T5 모델에 적용한 구현

이 모듈은 다음과 같은 특징을 가집니다:
1. 단일 디코더 레이어에만 DCG 적용 (마지막 레이어)
2. 단순화된 게이트 메커니즘
3. 점진적 DCG 활성화로 안정적인 학습
4. 경량화된 아키텍처
"""
...
CONFIG = {
    ...
    "output_dir": "./improved_dcg_model",  # 모델 저장 경로
    ...
    "train_size": 9000,                    # 훈련 데이터 크기
    "val_size": 1000,                      # 검증 데이터 크기
    "test_size": 1000,                     # 테스트 데이터 크기
    "gate_dropout": 0.1,                   # DCG 게이트 드롭아웃 비율
    "gate_regularization_weight": 0.01,    # 게이트 정규화 가중치
    "warmup_ratio": 0.1,                   # 워밍업 비율
    "gradient_accumulation_steps": 2,      # 경사 누적 단계
    "dcg_warmup_steps": 500,               # DCG 점진적 활성화 단계
}

class SimpleDynamicContextGates(nn.Module):
    """
    단순화된 Dynamic Context Gates
    소스 컨텍스트(인코더)와 타겟 컨텍스트(디코더)의 기여도를 조절하는 메커니즘
    """
    ...
    def forward(self, decoder_hidden, encoder_outputs, cross_attention_output):
        """DCG 순전파"""
        ...
        # 게이트 값 계산
        source_gate_weight = self.source_gate(gate_input)
        target_gate_weight = self.target_gate(gate_input)
        ...
        # 게이트 적용
        gated_source_context = source_gate_weight * encoder_mean
        ...
        # 잔차 연결 및 정규화
        output = self.layer_norm(fused_output + decoder_hidden)
        output = self.dropout(output)
        
        return output, source_gate_weight, target_gate_weight
...
class ImprovedT5WithDCG(T5ForConditionalGeneration):
    ...
    def compute_simple_gate_loss(self, gate_weights_source, gate_weights_target):
        """게이트 정규화 손실 계산"""
        if not gate_weights_source:
            return 0.0
            
        # 엔트로피 정규화 적용
        total_loss = 0.0
        for src_gate, tgt_gate in zip(gate_weights_source, gate_weights_target):
            ...
            total_loss += -(src_entropy + tgt_entropy)
        
        return total_loss / len(gate_weights_source)
    ...
    def forward(
        ...
        **kwargs,
        ...
        dcg_alpha = self.get_dcg_alpha()
        
        if decoder_outputs.hidden_states is not None and dcg_alpha > 0:
            # 마지막 hidden state에 DCG 적용
            last_hidden = decoder_outputs.hidden_states[-1]
            
            enhanced_hidden, src_gate, tgt_gate = self.dcg_module(
            ...
            # DCG 출력 점진적 적용
            sequence_output = (1 - dcg_alpha) * sequence_output + dcg_alpha * enhanced_hidden
        ...
        if labels is not None:
            ...
            if gate_weights_source and CONFIG["gate_regularization_weight"] > 0:
                gate_reg_loss = self.compute_simple_gate_loss(
                    gate_weights_source, gate_weights_target
                )
                loss += CONFIG["gate_regularization_weight"] * gate_reg_loss * dcg_alpha
...
def load_data():
    ...
    # 캐시 저장
    cache_data = {"train": train_data, "val": val_data, "test": test_data}
...
def train_improved_dcg_model():
    ...
    model = ImprovedT5WithDCG(config)
    ...
    try:
        ...
        with open(os.path.join(CONFIG["output_dir"], "train_results.json"), "w") as f:
            json.dump({
                ...
                "model_type": "T5_with_DCG",
                "gate_regularization_weight": CONFIG["gate_regularization_weight"],
                "dcg_warmup_steps": CONFIG["dcg_warmup_steps"]
            }, f, indent=2)
